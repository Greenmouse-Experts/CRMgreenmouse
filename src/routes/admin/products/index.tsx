      ...
      render: (val: any, item: Product) => (
        <div>
          <span className="font-medium text-base-content">
            {item.currency || "₦"}{Number(val || 0).toLocaleString()}
          </span>
          {item.cost ? (
            <div className="text-xs text-base-content/50">
              Cost: {item.currency || "₦"}{Number(item.cost).toLocaleString()}
            </div>
          ) : null}
        </div>
      ),
    ...
    <>
      ...
      >
        <div className="flex items-center gap-2">
          <Link to="/admin/products/categories" className="btn btn-outline btn-sm">
